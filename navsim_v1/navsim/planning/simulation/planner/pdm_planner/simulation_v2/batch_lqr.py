...
from navsim.planning.simulation.planner.pdm_planner.simulation_v2.batch_lqr_utils import (
...
class BatchLQRTracker:
    ...
    def _lateral_lqr_controller(
        ...
        for index_step, (state_matrix_at_step, affine_term) in enumerate(zip(states_matrix_at_step, affine_terms)):
            ...
            # affine_term (batch, 3)
            A = state_matrix_at_step @ A  # (batch, 3, 3)
            B = state_matrix_at_step @ B + in_matrix  # (batch, 3, 1)
            g = (state_matrix_at_step @ g[..., None]).squeeze(-1) + affine_term  # (batch, 3)
    ...
    def _solve_one_step_lateral_lqr(
        ...
        state_error_zero_input = (A @ initial_state[..., None]).squeeze(-1) + g
