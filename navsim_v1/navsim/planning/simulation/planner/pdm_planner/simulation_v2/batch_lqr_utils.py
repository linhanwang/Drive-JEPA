...
batch_matmul = lambda a, b: a @ b


# batched pseudo-inverse using scipy.linalg.pinv and np.vectorize over the batch dimension
def _batched_pinv(mats: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
    Apply scipy.linalg.pinv to a batch of matrices.
    :param mats: array of shape (B, M, N)
    :return: array of shape (B, N, M) with pinv applied per batch element
    """
    vec_pinv = np.vectorize(scipy_pinv, signature="(m,n)->(n,m)")
    return vec_pinv(mats)
...
def _fit_initial_velocity_and_acceleration_profile(
    ...
    intermediate_solution = batch_matmul(
        _batched_pinv(batch_matmul(A_T, A) + jerk_penalty * batch_matmul(R_T, R)), A_T
    )
    x = (intermediate_solution @ y[..., None]).squeeze(-1)
...
def _fit_initial_curvature_and_curvature_rate_profile(
    ...
    intermediate = batch_matmul(_batched_pinv(batch_matmul(A_T, A) + Q), A_T)
    x = (intermediate @ y[..., None]).squeeze(-1)
