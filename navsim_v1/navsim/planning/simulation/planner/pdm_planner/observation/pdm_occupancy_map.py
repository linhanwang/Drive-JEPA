...
class PDMDrivableMap(PDMOccupancyMap):
    ...
    def points_in_polygons_fast(self, points: npt.NDArray[np.float64]) -> npt.NDArray[np.bool_]:
        """
        Determines whether input-points are in polygons of the occupancy map (optimized version)
        Uses bounding box filtering to reduce the number of polygon checks.
        :param points: input-points
        :return: boolean array of shape (polygons, input-points)
        """
        assert points.shape[-1] == 2, "Points array must have shape (...,2) for x, y coordinates!"
        ...
        # Early exit if no points or no geometries
        if len(flattened_points) == 0 or len(self._geometries) == 0:
            output_shape = (len(self._geometries),) + input_shape
            return np.zeros(output_shape, dtype=bool)

        # Create bounding box of all points
        min_x, min_y = flattened_points.min(axis=0)
        max_x, max_y = flattened_points.max(axis=0)
        bbox = Polygon([(min_x, min_y), (max_x, min_y), (max_x, max_y), (min_x, max_y)])

        # Query polygons that intersect with the bounding box
        candidate_indices = self._str_tree.query(bbox, predicate="intersects")

        # Initialize output array
        output = np.zeros((len(self._geometries), len(flattened_points)), dtype=bool)

        # Only check points against candidate polygons
        for idx in candidate_indices:
            polygon = self._geometries[idx]
            output[idx] = shapely.vectorized.contains(
